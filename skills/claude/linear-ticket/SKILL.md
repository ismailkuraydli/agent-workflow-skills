---
name: linear-ticket
description: Read a Linear ticket end to end, ground it in the codebase, get a plan approved, then implement it in a git worktree on the ticket's branch with tests, moving the ticket to In Progress when work starts and In Review once the PR is up. Use when asked to work on, plan, pick up, or implement a Linear issue — "implement ISM-12", "take this ticket", or a linear.app issue URL. Pass --overnight with several tickets for an unattended run: one agent per ticket, parallel where independent, cascading PRs where they depend on each other, one approval up front and a report in the morning.
---
...
tickets, spawns one agent per ticket — parallel where they are independent,
...
The per-ticket work is still this file: each agent runs Phases 2–7 for its ticket.
...
Then call `EnterPlanMode` (skip if already in it). Everything in Phases 2–3 is
read-only and belongs there; the plan gets formal sign-off via `ExitPlanMode`.
...
Call `ExitPlanMode` and wait. Do not create the worktree before approval.

*Overnight:* there is one `ExitPlanMode` for the whole run sheet, at launch — see `OVERNIGHT.md` Phase O1. Nothing is approved per ticket after that.
...
DIR=".claude/worktrees/$(echo "$BRANCH" | tr '/' '-')"
...
Then `EnterWorktree` with `path: <absolute path to $DIR>` to move the session in.
...
### Shell constraints once you are inside

A worktree-isolated session refuses any Bash command it cannot statically prove
stays inside the worktree. In practice that rejects most of what you would reach for
by habit, with a "too complex to verify" error rather than a failure you can debug:

- **Heredocs are refused**, so `cat > file <<'EOF'` is not available for writing or
  appending. Use Write/Edit for every file change from here on — this is the case
  where a dedicated tool genuinely beats the shell, not a stylistic preference.
- **Compound commands are refused** — `&&` chains, `;` sequences, and command
  substitution spanning several steps. Run one command per call. This applies
  especially to Phase 6: each CI step is its own invocation, not one chained line.
- Simple single commands with redirection to a scratchpad path are fine, which is
  how to capture a long test run for grepping.

Hitting the refusal is not a permissions problem and re-phrasing the same compound
command rarely helps — split it instead.
...
you wrote — using the toolchain path found there, one command per call, each
redirected to a scratchpad log you then grep. Read the output rather than the exit
colour:
