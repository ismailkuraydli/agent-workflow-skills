...
- Linear MCP server configured (`claude mcp list` shows `linear`). If missing: `claude mcp add linear --url https://mcp.linear.app/mcp`
- A codebase to inspect — use `Grep` and `Read` to verify affected components and reuse opportunities
...
5. Identify **affected components**: use `Grep` and `Read` to find the files, modules, services, APIs that will be touched. Never guess — verify.
...
14. **Reuse-first audit:** For each planned new file/function/component, search the codebase with `Grep` for existing capabilities that already do this — existing utilities, config, framework built-ins, already-imported libraries. If something exists, switch the plan to use it.
...
- **Guessing affected components** — always verify with `Grep` or `Read`. Wrong components = wrong plan.
