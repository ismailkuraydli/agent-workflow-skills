...
metadata:
  hermes:
    tags: [code-review, security, tdd, minimal-change, pr, linear]
    related_skills: [plan-to-linear, linear-ticket, requesting-code-review]
---
...
The reviewer is a fresh subagent via `delegate_task`. It receives ONLY the diff,
...
```
delegate_task(
    goal=<reviewer prompt — see below>,
    context="Independent code review. Return only the review document.",
    output_schema=<JSON schema for structured verdict>
)
```
...
```
mcp__linear__create_comment(issueId=<ticket_id>, body=<review_document>)
```
