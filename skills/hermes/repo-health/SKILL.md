...
metadata:
  hermes:
    tags: [repo-health, refactoring, dead-code, decoupling, consistency, tech-debt, maintainability]
    related_skills: [repo-audit, plan-to-linear, code-review, codebase-inspection]
---
...
Each lens produces findings. Use `search_files` and `read_file` to verify — never report from grep alone.
