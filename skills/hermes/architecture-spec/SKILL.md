...
metadata:
  hermes:
    tags: [architecture, design, spec, adr, requirements, system-design]
    related_skills: [plan-to-linear, repo-health, codebase-onboarding]
---
...
- If existing system: codebase access via `search_files` and `read_file`
...
- What architecture is currently in place? (If codebase available, verify with
  `search_files` and `read_file` — don't trust a verbal description alone)
...
- **Not verifying the current system** — if a codebase exists, use `search_files`
  and `read_file` to verify the architecture. A verbal description is often
  wrong or incomplete.
