...
*orchestrator* level. Each spawned subagent still runs `SKILL.md` Phases 2–7 for its
own ticket.
...
|---|---|
| One ticket | Every ticket named, one subagent each |
...
3. **Check the integration branch right now**, not from the ticket text:
   ...
   not seven minutes into an subagent's baseline run.
4. **Find the toolchain and the verification commands once** (`SKILL.md` Phase 3),
   with absolute paths. Every subagent gets handed the result; none of them should
...
- the verification command set every subagent will run;
...
- A chain is one subagent handling its tickets in order, **not** a new subagent per link.
  One subagent already holds the context for A when it starts B, and re-briefing a
  fresh subagent onto a half-built stack is where stacks go wrong.
- Cap the fan-out. Past ~4–5 concurrent subagents the shared machine (CPU, one
...
**Isolation is the whole game.** Parallel subagents inherit the same paths by default
and overwrite each other silently — no error, just wrong results.

| Shared by default | Give each subagent |
|---|---|
| Worktree dir (named from the branch) | Its own — one ticket, one branch, one directory. Two subagents on one ticket is the single worst outcome. |
| Scratchpad dir | `scratchpad/<TICKET-ID>/` — otherwise one subagent's test log lands in another's file |
| The project's user-state / cache dir (keyed by project name, so every worktree shares it) | A per-subagent isolated one — the project's own override mechanism, or an isolated `HOME` for the test command only. **Never give git or `gh` the isolated `HOME`** — they lose the user's identity and auth. |
| Any single-instance local resource the suite needs | If the runtime genuinely allows one process at a time, serialise on an atomic `mkdir` lock acquired **per run, not per batch**, released in an `EXIT` trap. Do **not** make subagents wait on `pgrep` being empty — with other sessions running it never clears and the subagent stalls for the rest of the night. |

The brief for each subagent must stand alone — it cannot ask you anything:
...
- **"Stay on your own branch and worktree. Do not touch another subagent's directory,
  branch, or the integration branch."**
- **"Block on long runs inside your turn"** — background the run (`nohup … & echo $! >
  run.pid`) and `delegate_task(action='list')` or `process_manage(action='poll')` until it exits, rather than ending your turn to wait.
  An subagent that ends its turn mid-work is the thing that makes the rest of this
  protocol hard.
- its budget, and what to do on exhaustion (Phase O4).

## Phase O4 — Orchestrating while the user sleeps

**Never end your turn while subagents are running.** A completed child whose parent's
turn has ended is orphaned work. You own collection.

**A "finished" notification can be a pause.** It fires every time an subagent stops with
no live background children — including an subagent that backgrounded its own test run
and will be woken when it finishes. Before concluding an subagent is dead:
...
- a process check that finds *nothing* while other subagents are known to be testing is
  a broken check, not reassurance.

**Never run two subagents on one ticket.** If a replacement is genuinely needed:
`delegate_task(action='stop', subagent_id=...)` the original, confirm it stopped, *then* brief the replacement. Once two
subagents share a worktree, nothing in git can tell you which one is acting — the
...
(e.g. 3 red verification runs in a row). On exhaustion, the subagent stops cleanly:
...
branch, worktrees, and per-subagent state dirs. Deleting is outward-facing, and it is not
...
- Ending the turn with subagents still running, orphaning a night of work.
- Two subagents in one worktree — from a "finished" notification that was a pause.
- Parallel subagents clobbering each other's logs or shared state dir, producing test
