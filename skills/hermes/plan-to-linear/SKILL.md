...
metadata:
  hermes:
    tags: [planning, linear, tdd, bdd, bug, feature, ticket, minimal-change]
...
- Linear MCP tools available (`mcp__linear__create_issue`, `mcp__linear__get_teams`, `mcp__linear__get_workflow_states`)
- A codebase to inspect — use `search_files` and `read_file` to verify affected components and reuse opportunities
...
5. Identify **affected components**: use `search_files` and `read_file` to find the files, modules, services, APIs that will be touched. Never guess — verify.
...
14. **Reuse-first audit:** For each planned new file/function/component, search the codebase with `search_files` for existing capabilities that already do this — existing utilities, config, framework built-ins, already-imported libraries. If something exists, switch the plan to use it.
...
- **Guessing affected components** — always verify with `search_files` or `read_file`. Wrong components = wrong plan.
