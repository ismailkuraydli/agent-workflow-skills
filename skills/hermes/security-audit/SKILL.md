...
metadata:
  hermes:
    tags: [security, audit, cve, owasp, vulnerabilities, attack-surface, compliance]
    related_skills: [repo-audit, repo-health, code-review, github]
---
...
- `web_search` for CVE and best practice retrieval
...
13. **Read the actual implementation** — for each security-relevant feature, use
    `read_file` and `search_files` to read the full implementation. Understand:
    - What it does
...
25. **Check exploit availability** — for each CVE found:
    - Search ExploitDB: `web_search` "[CVE-ID] exploit"
    - Search GitHub: `web_search` "[CVE-ID] PoC site:github.com"
...
28. **Web search for best practices** — use `web_search` for each:
