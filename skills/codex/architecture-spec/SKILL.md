...
- If existing system: codebase access via `grep`/`rg` and file reads
...
- What architecture is currently in place? (If codebase available, verify with
  `grep`/`rg` and file reads — don't trust a verbal description alone)
...
- **Not verifying the current system** — if a codebase exists, use `grep`/`rg`
  and file reads to verify the architecture. A verbal description is often
  wrong or incomplete.
