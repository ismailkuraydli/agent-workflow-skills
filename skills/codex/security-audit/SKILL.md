...
- web search for CVE and best practice retrieval
...
13. **Read the actual implementation** — for each security-relevant feature, use
    file reads and `grep`/`rg` to read the full implementation. Understand:
    - What it does
...
25. **Check exploit availability** — for each CVE found:
    - Search ExploitDB: web search "[CVE-ID] exploit"
    - Search GitHub: web search "[CVE-ID] PoC site:github.com"
...
28. **Web search for best practices** — use web search for each:
