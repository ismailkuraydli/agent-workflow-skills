---
name: linear-ticket
description: "Read a Linear ticket, ground it in codebase, plan, implement in a worktree with TDD, push PR. Use when asked to implement a Linear issue. Pass --overnight for unattended multi-ticket runs."
---
...
tickets, launches one `codex exec` process per ticket — parallel where they are independent,
...
The per-ticket work is still this file: each `codex exec` process runs Phases 2–7 for its ticket.
...
Write the plan to a file (e.g. `<ticket-id>-plan.md`) and present it to the user.
Wait for explicit approval before proceeding — do not start Phase 4 until the
user says go. Everything in Phases 2–3 is read-only and belongs in the plan.
...
DIR=".codex/worktrees/$(echo "$BRANCH" | tr '/' '-')"
...
Then `cd "$DIR"` to enter the worktree.
